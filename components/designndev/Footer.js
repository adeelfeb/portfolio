    ...
    <div className="mt-5 max-w-sm">
      ...
      <form onSubmit={handleSubmit} className="relative flex flex-col gap-2 sm:flex-row sm:items-stretch">
        ...
        <input
          ...
          className="flex-1 min-w-0 rounded-lg border border-gray-700 bg-gray-800 px-3 py-2.5 text-sm text-white placeholder:text-gray-500 focus:border-emerald-500 focus:outline-none"
        />
        ...
        <button type="submit" disabled={status === 'loading'} className="shrink-0 rounded-lg bg-emerald-600 px-4 py-2.5 text-sm font-subheading text-white hover:bg-emerald-500 transition-colors disabled:opacity-60">
          {status === 'loading' ? '...' : 'Subscribe'}
        </button>
      </form>
      {message && <p className={`mt-2 text-xs font-subheading ${status === 'error' ? 'text-red-400' : 'text-emerald-400'}`} role="status">{message}</p>}
    </div>
  )
}
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 md:gap-10 flex-1 lg:max-w-3xl">
              <div>
                ...
                <ul className="space-y-2 text-sm font-subheading">
                  <li><Link href="/about-us" className="text-gray-400 hover:text-emerald-400 no-underline">About Us</Link></li>
                  <li><Link href="/faq" className="text-gray-400 hover:text-emerald-400 no-underline">FAQ</Link></li>
                </ul>
              </div>
              <div>
                <h4 className="font-subheading font-medium text-gray-400 text-xs tracking-wide uppercase mb-3">Features</h4>
                <ul className="space-y-2 text-sm font-subheading">
                  <li><Link href="/whatsapp-chat-analysis" className="text-gray-400 hover:text-emerald-400 no-underline">Chat Analytics</Link></li>
                  <li><Link href="/valentine" className="text-gray-400 hover:text-emerald-400 no-underline">Valentine</Link></li>
                </ul>
              </div>
              <div className="col-span-2 sm:col-span-1">
                <h4 className="font-subheading font-medium text-gray-400 text-xs tracking-wide uppercase mb-3">Quick Links</h4>
                <ul className="space-y-2 text-sm font-subheading">
                  <li><Link href="/signup" className="text-gray-400 hover:text-emerald-400 no-underline">Sign Up</Link></li>
                  <li><Link href="/login" className="text-gray-400 hover:text-emerald-400 no-underline">Sign In</Link></li>
                  <li><Link href="/dashboard" className="text-gray-400 hover:text-emerald-400 no-underline">Dashboard</Link></li>
                </ul>
              </div>
            </div>
          </div>
