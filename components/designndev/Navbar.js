...
function useRouterCompat() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const scheduleUpdate = () => queueMicrotask(() => setPathname(window.location.pathname))
      window.addEventListener('popstate', scheduleUpdate)
      const op = history.pushState
      const or = history.replaceState
      history.pushState = function (...args) { op.apply(history, args); scheduleUpdate() }
      history.replaceState = function (...args) { or.apply(history, args); scheduleUpdate() }
      return () => {
          ...
          >
            <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 pb-16">
              <div className="sm:hidden mb-6 flex gap-3">
                <Link href="/login" className="flex-1 text-center text-base font-medium text-gray-700 border border-gray-200 rounded-xl py-3.5 no-underline" onClick={() => setIsMenuOpen(false)}>
                  Sign In
                </Link>
                <Link href="/signup" className="btn-fc-primary flex-1 text-center text-base py-3.5 no-underline" onClick={() => setIsMenuOpen(false)}>
                  Sign Up
                </Link>
              </div>
