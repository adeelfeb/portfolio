 ...
 * When importingUserId is provided, Valentine URLs are remapped so their
 * createdBy references the correct user in the current database.
 * @param {Object} data - { [collectionKey]: Array }
 * @param {ObjectId|null} importingUserId - _id of the user performing the import
 * @returns {Promise<Object>} { [collectionKey]: { inserted, skipped, errors } }
 */
export async function applyImport(data, importingUserId = null) {
  await connectDB();
  const results = {};
  const keyToModel = Object.fromEntries(BACKUP_MODELS.map(({ key, model }) => [key, model]));

  const userOldIdToNewId = new Map();
  ...
  for (const { key, model } of BACKUP_MODELS) {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      const doc = { ...row };
      const oldId = doc._id ? String(doc._id) : null;
      delete doc._id;
      delete doc.__v;
      if (key === 'users' && !doc.password) doc.password = 'ImportedBackup1!';

      if (key === 'valentineurls') {
        const createdByStr = doc.createdBy ? String(doc.createdBy) : null;
        if (createdByStr && userOldIdToNewId.has(createdByStr)) {
          doc.createdBy = userOldIdToNewId.get(createdByStr);
        } else if (importingUserId) {
          doc.createdBy = importingUserId;
        }
        if (doc.slug) {
          let slug = doc.slug;
          let counter = 0;
          while (await ValentineUrl.findOne({ slug })) {
            counter++;
            slug = `${doc.slug}-imported-${counter}`;
          }
          doc.slug = slug;
        }
      }
      ...
      try {
        ...
        if (key === 'users' && oldId) {
          userOldIdToNewId.set(oldId, created._id);
        }
      } catch (err) {
        if (key === 'users' && oldId && doc.email) {
          try {
            const existing = await User.findOne({ email: doc.email });
            if (existing) userOldIdToNewId.set(oldId, existing._id);
